<template>
  <div id="app">
    ...
    <div class="app-body">
      <div class="app-body-tool">
        ...
        <div>
          <el-button
            type="primary"
            @click="handleSelection('addForm')"
            size="small"
            >全选新增表单</el-button
          >
          <el-button
            type="primary"
            @click="handleSelection('searchForm')"
            size="small"
            >全选搜索表单</el-button
          >
          <el-button
            type="primary"
            @click="handleSelection('queryForm')"
            size="small"
            >全选查询表单</el-button
          >
          <el-button
            type="primary"
            @click="handleSelection('isRequired')"
            size="small"
            >全选是否必填</el-button
          >
        </div>
    ...
    <el-dialog
      :title="isConnect ? '连接MYSQL' : '切换MYSQL'"
      :show-close="!isConnect"
    ...
    >
      ...
      >
        <el-form-item label="生成文件的模板" prop="templateFile">
          <el-input
            v-model="generateForm.templateFile"
            readonly
            @click.native="
              vsPossMessage('getTemplateUrl', {
                canSelectFiles: true,
                filters: { TypeScript: ['ejs'] },
        ...
        <el-form-item label="生成到指定目录" prop="path">
          <el-input
            v-model="generateForm.path"
            readonly
            @click.native="vsPossMessage('getFolderUrl')"
            placeholder="请选择文件路径"
          >
          </el-input>
        </el-form-item>
        <el-form-item label="生成文件的名称" prop="fileName">
          <el-input
            ...
            placeholder="请输入文件名称(*.vue)"
          ></el-input>
        </el-form-item>
        <el-form-item label="其它配置">
          <span class="other-config-tip"
            >(只能在默认配置上新增配置不能删除默认配置，属性没有值就为空字符串)</span
          >
          <vue-json-editor
            class="json-editor"
            @has-error="isJosn = false"
            v-model="otherConfig"
            @json-change="isJosn = true"
            :showBtns="false"
            :mode="'code'"
          />
        </el-form-item>
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="handleGenerateSubmit"
          >确 定</el-button
        >
      </span>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    handleGenerate() {
      ...
      if (
        !obj.queryList.length &&
        !obj.searchList.length &&
        !obj.insertList.length &&
        !obj.requiredList.length
      ) {
